  ...
  try {
    ...
    if (Math.abs(transaction.amount) >= settings.spendingLimitAmount) {
      await createNotification(
        userId,
        "budget",
        `Alerta de gasto: Transação de ${formatCurrency(
          Math.abs(transaction.amount)
        )} excede seu limite de ${formatCurrency(
          settings.spendingLimitAmount
        )}.`,
        {
    ...
    goalsSnapshot.docs.forEach(async (doc) => {
      ...
      if (
        ...
        const daysRemaining = Math.ceil(
          (targetDate - today) / (24 * 60 * 60 * 1000)
        );
        ...
        await createNotification(
          ...
          `Meta "${
            goal.title
          }" vence em ${daysRemaining} dias e está ${Math.round(
...
export const checkRecurringTransactions = async (userId) => {
  try {
    ...
    Object.values(transactionGroups).forEach(async (transactions) => {
      ...
      const nextInstallment = transactions.find(
      ...
      if (nextInstallment) {
        ...
        if (installmentDate - today <= threeDays) {
          const daysRemaining = Math.ceil(
            (installmentDate - today) / (24 * 60 * 60 * 1000)
          );

          await createNotification(
            userId,
            "transaction",
            `Lembrete: Parcela ${nextInstallment.installmentNumber}/${
              nextInstallment.totalInstallments
            } de "${nextInstallment.description
              .split("(")[0]
              .trim()}" vence em ${daysRemaining} dias.`,
            {
